...
const sourcePath = resolve(__dirname, 'proto')
...
const tempPath = resolve(__dirname, '_tmp_')
...
function generateFiles() {
   ...
   * ```
   * */
  const option_useOptionsls = `--ts_proto_opt=useOptionals=true`

  /** выбор имплементации `grpc`-сервисов */
  const option_outputServices = `--ts_proto_opt=outputServices=grpc-js`
  ...
  run(
    ...
    option_oneof,
    //@ts-ignore
    ...glob.sync(resolve(sourcePath, '**/*.proto'))
  )
}
